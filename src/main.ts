...
function boot() {
  ...
  const translate = (root: Element, route = parseRoute(location.href)) => { translateRoot(root, route); translateTitles(root, service); void translateBangumiTags(root, tagService); chineseSearch.refresh(); };
  const onRoute = (route: ReturnType<typeof parseRoute>) => { if (document.body) translate(document.body, route); };
