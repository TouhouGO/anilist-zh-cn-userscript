...
describe('userscript metadata', () => {
  it('defines the public install and update metadata in one canonical block', () => {
    expect(userscriptVersion).toBe('0.1.20');
    ...
    expect(userscriptMetadata).toContain('// @grant        GM_getValue');
    expect(userscriptMetadata).toContain('// @grant        GM_setValue');
    ...
    expect(userscriptMetadata).toContain('// @connect      raw.githubusercontent.com');
  });
  ...
  it('replaces an outdated metadata block instead of creating a duplicate', () => {
    ...
    expect(result).toContain('// @version      0.1.20');
